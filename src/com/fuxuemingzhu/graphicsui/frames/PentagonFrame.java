...
import com.fuxuemingzhu.graphicsui.alerts.PentagonAnswerAlert;
import com.fuxuemingzhu.graphicsui.draws.PentagonDraw;
import com.fuxuemingzhu.graphicsui.io.PentagonIO;
...
public class PentagonFrame extends JFrame {
	...
	public void btnsub_ActionEvent(ActionEvent e) {
		...
		if (txt_r.equals("")) {
			...
			PentagonAnswerAlert circleAnswerAlert = new PentagonAnswerAlert(
					Float.parseFloat(txt_r));// 构造结果显示框
			PentagonDraw circleDraw = new PentagonDraw(Float.parseFloat(txt_r));// 构造图形绘制框
			PentagonIO pentagonIO = new PentagonIO(path,
					Float.parseFloat(txt_r));
			// /////////////////构造结果输出
			try {
				pentagonIO.writeFile();
			} catch (Exception e1) {
